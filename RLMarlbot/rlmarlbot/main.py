...
RLMARLBOT_PARENT = os.path.abspath(os.path.join(CURRENT_DIR, ".."))
for p in [RLMARLBOT_PARENT, CURRENT_DIR]:
...
class RLMarlbot:
    ...
    def start(self):
        ...
        if self.use_uinput:
            ...
            self.vc = VirtualXboxController()
            self.mw = None
            print(Fore.LIGHTGREEN_EX + "Virtual Gamepad (uinput) initialized (crash-free native input)" + Style.RESET_ALL)
        else:
    ...
    def on_tick(self, event: EventPlayerTick):
        ...
        try:       
            ...
            try:
                ...
                if self.use_uinput:
                    on_ground = bool(self.local_car.is_on_ground())
                    self.vc.apply_action(controller_state, on_ground=on_ground)
                    self.debug_info("Inputs sent to virtual gamepad")
                else:
    ...
    def stop_writing(self):
        ...
        if self.input_address and self.mw:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clock", action="store_true", default=True, help="Sync ticks with an internal clock at 120Hz (default on Linux)")
    parser.add_argument("--debug", action="store_true", help="Show debug information in the console")
    parser.add_argument("--memory", action="store_true", help="Use direct memory writing (process_vm_writev) instead of virtual gamepad")
    parser.add_argument("--start-active", action="store_true", help="Start playing immediately")
